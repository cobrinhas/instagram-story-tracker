...
import { renderFoundNewStoryActivityMessage, renderStoryAlertCaption } from "../bot/templates";
...
export async function handleStoryActivityTracking(
    ...
    logMessage(`story activity tracking started... ${records.length} records to go through`);

    for (const [chatID, trackRecord] of records) {
        const previousTrackedStories = trackRecord.trackedStories;
        const fetchedStories = await userRepository.stories(trackRecord.trackingUser.userId);

        const storiesLeftToAnnounce = fetchedStories.filter((ft) => previousTrackedStories.find((st) => ft.id == st.id) === undefined);
        trackRecord.trackedStories.push(...storiesLeftToAnnounce);

        logMessage(`found ${storiesLeftToAnnounce.length} left to announce for user: ${chatID}`)

        if (storiesLeftToAnnounce.length > 0) {
            await telegram.sendMessage(chatID, renderFoundNewStoryActivityMessage(trackRecord.trackingUser.username), { parse_mode: 'Markdown' });
        }

        for (const story of storiesLeftToAnnounce) {
            if (story.isVideo) {
                telegram.sendVideo(chatID, story.url, { caption: renderStoryAlertCaption(story) })
            } else {
                telegram.sendPhoto(chatID, story.url, { caption: renderStoryAlertCaption(story) })
            }
        }
    }
}
